...
export class ExamRepository {
    ...
    async createUserAnswer(tx: any, payload: { session_id: number, question_id: number, choice_id: number}) {
        return tx.userAnswer.create({ data: payload });
    }

    async updateSessionPostion(tx: any, sessionId: number, data: Partial<{ current_position: number, completed_at: Date }>) {
        return tx.userExamSession.update({ where: { id: sessionId }, data });
    } 

    async findQuestionWithChoices(questionId: number) {
        return this.prismaService.question.findUnique({
            where: { id: questionId }, include: { question_choices: true }
        });
    }
    ...
    async updateSessionPositionRaw(sessionId: number, data: any) {
        ...
        if (data.current_position !== undefined) safeData.current_position = data.current_position;
