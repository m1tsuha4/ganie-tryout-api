...
export class ExamService {
    ...
    async submitAnswer(sessionId: number, userId: string, questionId: number, choiceId: number, index?: number) {
    const session = await this.repo.findSessionById(sessionId);
    if (!session) {
        throw new ForbiddenException('Session not found');
    }
    ...
    if (index !== undefined) {
        ...
        throw new BadRequestException('QuestionId does not match the provided index');
        }
    } else {
        idx = qOrder.findIndex((q) => q === questionId);
        if (idx === -1) throw new BadRequestException('Question does not belong to this session');
    }
    ...
    try {
        ...
        if (existing) {
            ...
            data: { session_id: sessionId, question_id: questionId, choice_id: choiceId },
            });
        }
        ...
        await tx.userExamSession.update({ where: { id: sessionId }, data: updateData });
        });
    } finally {
